...
interface DiffLine { type: "add" | "del" | "normal" | "hunk"; content: string; oldNum?: number; newNum?: number }
...
function computeDiffLines(before: string, after: string, path: string): DiffLine[] {
  ...
  for (const hunk of patch.hunks) {
    ...
    let oldNum = hunk.oldStart; let newNum = hunk.newStart
    for (const line of hunk.lines) {
      if (line.startsWith("+")) lines.push({ type: "add", content: line.slice(1), newNum: newNum++ })
      else if (line.startsWith("-")) lines.push({ type: "del", content: line.slice(1), oldNum: oldNum++ })
      else lines.push({ type: "normal", content: line.slice(1), oldNum: oldNum++, newNum: newNum++ })
    }
  }
  return lines
}
...
function DiffStats({ before, after }: { before: string; after: string }) {
  const stats = useMemo(() => {
    ...
    let add = 0, del = 0
    for (const hunk of patch.hunks) { for (const line of hunk.lines) { if (line.startsWith("+")) add++; else if (line.startsWith("-")) del++ } }
    return { add, del }
  }, [before, after])
  return (
...
function DiffView({ before, after, path }: { before: string; after: string; path: string }) {
  ...
  return (
    <div className="oac-diff-view font-mono" style={{ fontSize: "12px" }}>
      {lines.map((line, i) => (
        <div key={i} className={`oac-diff-line ${line.type === "add" ? "oac-diff-add" : line.type === "del" ? "oac-diff-del" : line.type === "hunk" ? "oac-diff-hunk" : ""}`}>
          ...
          <span className="oac-diff-sign">{line.type === "add" ? "+" : line.type === "del" ? "-" : line.type === "hunk" ? "" : " "}</span>
      ...
      {fileDiffs.length === 0 ? (
        ...
        <>
          ...
          <div className="flex-1 min-h-0 overflow-auto">
            {selectedDiff && <DiffView path={selectedDiff.path} before={selectedDiff.diff.before ?? ""} after={selectedDiff.diff.after} />}
          </div>
        </>
      )}
    </div>
