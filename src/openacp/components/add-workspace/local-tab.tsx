...
export function LocalTab(props: LocalTabProps) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="text-sm font-medium text-foreground-weaker uppercase tracking-wider mb-3">Workspaces on this machine</p>
        {loading && <p className="text-base font-normal text-muted-foreground py-2">Looking for workspaces...</p>}
        {!loading && instances.length === 0 && <p className="text-base font-normal text-muted-foreground py-2">No workspaces found on this machine.</p>}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-0.5">
                      <p className="text-sm font-normal text-muted-foreground truncate"><span className="text-foreground-weaker">Folder </span>{inst.directory}</p>
                      {inst.port ? (
                        <p className="text-sm font-normal text-muted-foreground"><span className="text-foreground-weaker">Address </span><span className="font-mono">http://localhost:{inst.port}</span></p>
                      ) : (
                        <p className="text-sm font-normal text-foreground-weaker">Not running</p>
                      )}
                    </div>
                  </div>
                  <Badge
      ...
      <div className="border-t border-border pt-5">
        <p className="text-sm font-medium text-foreground-weaker uppercase tracking-wider mb-3">Open a folder</p>
        <Button
          type="button"
          variant="outline"
          onClick={handleBrowse}
          className="w-full px-4 py-3 rounded-xl h-auto justify-start gap-3"
        >
          <svg width="16" height="16" viewBox="0 0 20 20" fill="none" className="shrink-0"><path d="M2.5 5.83333V15.8333H17.5V7.5H9.58333L7.5 5.83333H2.5Z" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" /></svg>
...
function BrowseResultView(props: { result: BrowseResult; instances: InstanceListEntry[]; onAdd: (e: WorkspaceEntry) => void; onSetup?: (path: string, instanceId: string) => void; onClose: () => void }) {
  ...
  if (result.type === 'known') {
    ...
    return (
      <div className="p-4 bg-secondary rounded-xl border border-border space-y-3">
        <div><p className="text-base font-medium text-foreground mb-1">Workspace found</p><p className="text-sm font-normal text-muted-foreground">This folder is already set up as <strong className="text-foreground-weak">{inst.name ?? inst.id}</strong>. Click Add to open it here.</p></div>
        <div className="flex items-center gap-2">
          <Button
            type="button"
            onClick={() => props.onAdd({ id: inst.id, name: inst.name ?? inst.id, directory: inst.directory, type: 'local' })}
            className="px-4 py-1.5 text-sm font-medium h-auto"
          >
            Add workspace
          </Button>
          <Button type="button" variant="ghost" onClick={props.onClose} className="text-sm font-normal text-muted-foreground h-auto">Back</Button>
        </div>
      </div>
    )
  }
  if (result.type === 'unregistered') {
    return (
      <div className="p-4 bg-secondary rounded-xl border border-border space-y-3">
        <div><p className="text-base font-medium text-foreground mb-1">Existing workspace detected</p><p className="text-sm font-normal text-muted-foreground">This folder already has an OpenACP workspace. Click Add to register it.</p></div>
        <div className="flex items-center gap-2">
