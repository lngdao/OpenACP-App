...
import { discoverWorkspaces, type InstanceInfo } from "../api/workspace-store"
...
export function WelcomeScreen({
  ...
  const [discovered, setDiscovered] = useState<InstanceInfo[]>([])

  useEffect(() => {
    let cancelled = false
    discoverWorkspaces().then((result) => {
            ...
            <div className="flex flex-col gap-1">
              {discovered.map((instance) => (
                <button
                  ...
                  onClick={() => onSelectWorkspace(instance.id)}
                >
                  <span className="text-14-medium text-text-strong">{dirName(instance.workspace)}</span>
                  <span className="text-12-regular text-text-weak truncate">{shortPath(instance.workspace)}</span>
                </button>
              ))}
            </div>
          </div>
