...
export function InstallScreen({ onSuccess }: Props) {
    ...
    <div className="flex h-screen w-screen flex-col items-center justify-start bg-background-base p-8 pt-16">
      <div className="flex w-full max-w-[520px] flex-col items-center gap-6">
        ...
        <div className="flex flex-col items-center gap-2">
          <h1 className="text-20-medium text-text-strong">Installing OpenACP</h1>
          <p className="text-14-regular text-text-weak">
        ...
        <div
          ref={logRef}
          className="h-[200px] w-full overflow-y-auto rounded-lg bg-[#1a1a1a] p-4 text-12-mono text-[#a3a3a3]"
        >
          {lines.map((line, i) => (
            <div key={i} dangerouslySetInnerHTML={{ __html: ansiToHtml(line) }} />
          ))}
          {status === 'running' && (
            <span className="animate-pulse text-[#737373]">{'\u258C'}</span>
          )}
        </div>
        ...
        <div className="flex w-full flex-col gap-2">
          <div className="flex w-full items-center justify-between">
            ...
            <span className="text-12-medium text-text-strong">
              {progressPercent}%
            </span>
          </div>
          <div className="h-2 w-full overflow-hidden rounded-full bg-surface-raised-base">
            <div
              className={`h-full rounded-full transition-all duration-300 ${
                status === 'success' ? 'bg-surface-success-strong' :
                status === 'error' ? 'bg-surface-critical-strong' :
                'bg-text-strong'
              }`}
        ...
        {status === 'success' && (
          <div className="flex w-full items-center justify-between rounded-lg border border-border-base bg-surface-raised-base px-4 py-3">
            ...
            <button
              onClick={() => onSuccess(configExists)}
              className="text-14-medium rounded-md bg-text-strong px-4 py-2 text-background-stronger transition-opacity hover:opacity-90"
            >
              Get Started {'\u2192'}
            </button>
          </div>
        ...
        {status === 'error' && (
          <div className="w-full rounded-lg border border-surface-critical-strong bg-surface-raised-base p-4">
            <p className="text-14-medium mb-1 text-surface-critical-strong">Installation Failed</p>
            <p className="text-14-regular mb-4 text-surface-critical-strong">{error}</p>
            <div className="flex justify-end gap-3">
              <button
                onClick={copyCommand}
                className="text-14-medium rounded-md border border-border-base bg-background-stronger px-4 py-2 text-text-strong transition-colors hover:bg-surface-raised-base-hover"
              >
                Copy command
              </button>
              <button
