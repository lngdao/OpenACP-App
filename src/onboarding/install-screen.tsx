  ...
  return (
    <div className="flex h-screen w-screen flex-col items-center justify-start bg-background-base p-8 pt-16">
      <div className="flex w-full max-w-[520px] flex-col items-center gap-6">
        ...
        <div className="flex flex-col items-center gap-2"><h1 className="text-20-medium text-text-strong">Installing OpenACP</h1><p className="text-14-regular text-text-weak">Setting up the OpenACP CLI on your system...</p></div>
        ...
        <div className="flex w-full flex-col gap-2">
          <div className="flex w-full items-center justify-between"><span className="text-12-regular text-text-weak">{status === 'success' ? 'Completed' : status === 'error' ? 'Failed' : 'Installing...'}</span><span className="text-12-medium text-text-strong">{progressPercent()}%</span></div>
        ...
        {status === 'success' && (
          <div className="flex w-full items-center justify-between rounded-lg border border-border-base bg-surface-raised-base px-4 py-3">
            <span className="text-14-regular text-surface-success-strong">OpenACP installed successfully</span>
            <button onClick={() => props.onSuccess(configExists)} className="text-14-medium rounded-md bg-text-strong px-4 py-2 text-background-stronger transition-opacity hover:opacity-90">Get Started</button>
          </div>
        )}
        {status === 'error' && (
          <div className="w-full rounded-lg border border-surface-critical-strong bg-surface-raised-base p-4">
            <p className="text-14-medium mb-1 text-surface-critical-strong">Installation Failed</p><p className="text-14-regular mb-4 text-surface-critical-strong">{error}</p>
            <div className="flex justify-end gap-3">
              <button onClick={copyCommand} className="text-14-medium rounded-md border border-border-base bg-background-stronger px-4 py-2 text-text-strong transition-colors hover:bg-surface-raised-base-hover">Copy command</button>
              <button onClick={runInstall} className="text-14-medium rounded-md bg-text-strong px-4 py-2 text-background-stronger transition-opacity hover:opacity-90">Retry</button>
            </div>
          </div>
        )}
      </div>
