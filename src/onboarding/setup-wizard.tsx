          ...
          <div className="flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex h-10 items-center gap-2 rounded-md border border-border-base px-3">
                ...
                <input type="text" value={workspace} onChange={(e) => setWorkspace(e.target.value)} placeholder="/Users/you/projects" className="text-14-regular min-w-0 flex-1 bg-transparent text-text-strong outline-none placeholder:text-text-weak" />
                <button onClick={async () => { const s = await openDialog({ directory: true, multiple: false }); if (s && typeof s === 'string') setWorkspace(s); }} className="text-14-medium shrink-0 text-text-interactive-base hover:underline">Browse</button>
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-14-medium text-text-strong">Select an AI agent</label>
              {agentInstallError && <p className="text-12-regular text-surface-critical-strong">{agentInstallError}</p>}
              {agentsLoading && <p className="text-14-regular py-4 text-text-weak">Loading agents...</p>}
              {agentsError && <><p className="text-14-regular mb-2 text-surface-critical-strong">Failed to load agents</p></>}
              {!agentsLoading && !agentsError && (
                <>
                  {agents.length > 4 && <input type="text" value={agentSearch} onChange={(e) => setAgentSearch(e.target.value)} placeholder="Search agents..." className="text-14-regular mb-1 h-10 w-full rounded-md border border-border-base bg-transparent px-3 text-text-strong outline-none placeholder:text-text-weak" />}
                  <div className="flex max-h-64 flex-col gap-1 overflow-y-auto pr-1">
                    {filteredAgents.map((agent) => (
                      <div key={agent.key} className={`flex cursor-pointer items-center gap-3 rounded-lg border px-4 py-3 transition-colors ${selectedAgent === agent.key ? 'border-text-strong border-2 bg-surface-raised-base' : 'border-border-base hover:bg-surface-raised-base-hover'}`} onClick={() => agent.installed && setSelectedAgent(agent.key)}>
                        ...
                        <div className="min-w-0 flex-1"><p className="text-14-medium text-text-strong">{agent.name}</p><p className="text-12-regular text-text-weak">{agent.description}</p></div>
                        {agent.installed && <span className="shrink-0 rounded-full bg-[#16a34a22] px-2 py-0.5 text-[11px] text-[#16a34a]">Installed</span>}
                        {!agent.installed && agent.available && <button onClick={(e) => { e.stopPropagation(); installAgent(agent.key); }} disabled={installingAgent === agent.key} className="text-12-medium shrink-0 rounded-md border border-border-base px-3 py-1 text-text-strong transition-colors hover:bg-surface-raised-base-hover disabled:opacity-50">{installingAgent === agent.key ? 'Installing...' : 'Install'}</button>}
                      </div>
                    ))}
                    {filteredAgents.length === 0 && <p className="text-14-regular py-4 text-center text-text-weak">No agents match "{agentSearch}"</p>}
                  </div>
            ...
            <button onClick={() => setStep(2)} disabled={!canProceedStep1} className="text-14-medium h-11 w-full rounded-lg bg-text-strong text-background-stronger transition-opacity hover:opacity-90 disabled:opacity-40">Continue</button>
          </div>
        )}
        {step === 2 && (
          <div className="flex flex-col gap-8">
            <div className="flex flex-col gap-2"><h1 className="text-20-medium text-text-strong">Confirm your setup</h1><p className="text-14-regular text-text-weak">Review your configuration before completing setup.</p></div>
            <div className="flex flex-col rounded-lg border border-border-base bg-surface-raised-base">
              <div className="flex items-center justify-between px-5 py-4"><span className="text-14-regular text-text-weak">Workspace</span><span className="text-14-medium text-text-strong">{workspace}</span></div>
              <div className="mx-5 h-px bg-border-base" /><div className="flex items-center justify-between px-5 py-4"><span className="text-14-regular text-text-weak">Agent</span><span className="text-14-medium text-text-strong">{selectedAgent}</span></div>
              <div className="mx-5 h-px bg-border-base" /><div className="flex items-center justify-between px-5 py-4"><span className="text-14-regular text-text-weak">Run mode</span><span className="text-14-medium text-text-strong">Daemon (background)</span></div>
            </div>
            ...
            {setupStatus === 'error' && <div className="rounded-lg border border-surface-critical-strong p-4"><p className="text-14-regular text-surface-critical-strong">{setupError}</p></div>}
            <div className="flex justify-end gap-3">
              <button onClick={() => setStep(1)} disabled={setupStatus === 'running' || setupStatus === 'starting'} className="text-14-medium rounded-md border border-border-base bg-background-stronger px-4 py-2.5 text-text-strong shadow-xs transition-colors hover:bg-surface-raised-base-hover disabled:opacity-40">Back</button>
              <button onClick={runSetup} disabled={setupStatus === 'running' || setupStatus === 'starting' || setupStatus === 'success'} className="text-14-medium flex items-center gap-2 rounded-md bg-text-strong px-6 py-2.5 text-background-stronger transition-opacity hover:opacity-90 disabled:opacity-40">
...
function StepDot(props: { active: boolean; done: boolean; label: string }) {
  return (
    <div className={`text-14-medium flex h-8 w-8 items-center justify-center rounded-full ${props.done ? 'bg-surface-success-strong text-white' : props.active ? 'bg-text-strong text-background-stronger' : 'bg-surface-raised-base border border-border-base text-text-weak'}`}>
