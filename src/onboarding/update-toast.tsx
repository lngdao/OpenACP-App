...
export function UpdateToasts() {
  ...
  return (
    <div className="pointer-events-none fixed bottom-4 right-4 z-50 flex flex-col gap-2">
      {coreUpdate && (
        <Toast icon={<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--text-strong)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/></svg>}
      ...
      {appUpdateAvailable && (
        <Toast icon={<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--text-strong)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="20" height="14" x="2" y="3" rx="2"/><line x1="8" x2="16" y1="21" y2="21"/><line x1="12" x2="12" y1="17" y2="21"/></svg>}
  ...
  return (
    <div className="pointer-events-auto flex w-[340px] items-center gap-3 rounded-lg border border-border-base bg-surface-raised-stronger px-4 py-3.5 shadow-lg">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-12-medium text-text-strong">{props.title}</p><p className="text-12-regular text-text-weak">{props.description}</p>
        {props.error && <p className="text-12-regular mt-1 text-surface-critical-strong">{props.error}</p>}
      </div>
      <button onClick={props.onUpdate} disabled={props.loading} className="text-12-medium shrink-0 rounded-md bg-text-strong px-3.5 py-1.5 text-background-stronger transition-opacity hover:opacity-90 disabled:opacity-50">{props.loading ? '...' : 'Update'}</button>
